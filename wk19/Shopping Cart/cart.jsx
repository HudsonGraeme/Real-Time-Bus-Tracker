        ...
        return (
            <li key={index}>
                ...
                <Button
                    ...
                    name={item.name}
                    onClick={addToCart}
                    disabled={cartCounts[item.name] && cartCounts[item.name] >= item.instock}
                >
                    ${item.cost} {item.name} ({cartCounts[item.name] ? item.instock - cartCounts[item.name] : item.instock} in stock)
                </Button>
            </li>
        );
    });
    let cartList = Object.entries(cartCounts).map(([name, count], index) => {
        const { cost, country } = products.find(
                ...
                if (nameMap.includes(item.name)) {
                    return {
                        ...
                        instock:
                            ...
                            parseInt(
                                existingItems[nameMap.indexOf(item.name)]
                                    .instock
                            ),
                    };
                }
